...
#covg = [181, 273, 673, 110, 259, 57, 126, 145, 209, 227, 132, 70, 91, 83, 481, 88, 120, 407, 863, 205, 193, 111, 92, 140, 140, 144, 110, 50, 100, 75] # this is 30 covg values that are close to magtre hodg. values
...
covg = sorted(covg) # this sorts the coverage list so it will be more even, though idk if it's necessary
#covg = sorted(covg, reverse=True) # this sorts the coverage list (in reverse) so it will be more even, though idk if it's necessary
print covg
unique = numpy.unique
...
def min_1(num_hodg, num_cells, num_eggs_laid, covg, temp_output):
	...
	for x in xrange(0, num_eggs_laid):
		present = 0
		#genome = weighted_pick(covg, num_cells)
	...
	#out.write("%s %s %s\n" % (num_hodg, num_cells, float(viable) / num_eggs_laid))
	#temp_output.put(viable_list)
...
def test_num_hodg(*list_of_indices):
	...
	for index in list_of_indices:
		...
		slice_covg = [(float(item) / new_sum) for item in slice_covg] # List comprehension ftw!
		for y in xrange(1, (num_cells + 1), 20):
			#present_list = [0] * num_eggs_laid
			#viable_list = min_1((index + 1), y, num_eggs_laid, slice_covg, temp_output)
			...
			if viable_list[2] == 1.0:
				...
				break
			#print percents
			#heat_map_matrix[x].append(new_row[:])
